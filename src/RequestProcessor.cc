...
void
RequestProcessor::sendInBody(CURL *curl, struct curl_slist *&slist, const std::string &toSend) const {
    // This avoids the Expect: 100 continue
    // Which is generated by curl when the body is long
...
std::string *
RequestProcessor::sendDupFormat(CURL *curl, const RequestInfo &rInfo, struct curl_slist *&slist) const {
